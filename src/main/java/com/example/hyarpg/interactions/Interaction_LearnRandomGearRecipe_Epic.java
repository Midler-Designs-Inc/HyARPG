...
public class Interaction_LearnRandomGearRecipe_Epic extends SimpleInstantInteraction {
    // Create the CODEC - this is required for serialization
    public static final BuilderCodec<Interaction_LearnRandomGearRecipe_Epic> CODEC = BuilderCodec.builder(
            Interaction_LearnRandomGearRecipe_Epic.class,
            Interaction_LearnRandomGearRecipe_Epic::new,
    ...
    protected void firstRun(
        ...
        // filter the players discovered recipes down to only epics
        List<String> epicRecipes = craftingKnowledge.discoveredDroppableRecipes.stream()
            .filter(s -> s.contains("_Epic"))
            .toList();
        ...
        List<String> commonRecipes = craftingKnowledge.discoveredDroppableRecipes.stream()
            .filter(
                ...
                && !epicRecipes.contains(s.replace("_Common", "_Epic"))
            )
            .toList();
        ...
        craftingKnowledge.addDiscoveredRecipe(entityRef, store, randomRecipe.replace("_Common", "_Epic"));
    }
}
