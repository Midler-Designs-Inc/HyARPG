...
public class Interaction_LearnRandomGearRecipe_Legendary extends SimpleInstantInteraction {
    ...
    public static final BuilderCodec<Interaction_LearnRandomGearRecipe_Legendary> CODEC = BuilderCodec.builder(
            Interaction_LearnRandomGearRecipe_Legendary.class,
            Interaction_LearnRandomGearRecipe_Legendary::new,
    ...
    protected void firstRun(
        ...
        // filter the players discovered recipes down to only legendary
        List<String> legendaryRecipes = craftingKnowledge.discoveredDroppableRecipes.stream()
            .filter(s -> s.contains("_Legendary"))
            .toList();
        ...
        List<String> commonRecipes = craftingKnowledge.discoveredDroppableRecipes.stream()
            .filter(
                ...
                && !legendaryRecipes.contains(s.replace("_Common", "_Legendary"))
            )
            .toList();
        ...
        craftingKnowledge.addDiscoveredRecipe(entityRef, store, randomRecipe.replace("_Common", "_Legendary"));
    }
}
