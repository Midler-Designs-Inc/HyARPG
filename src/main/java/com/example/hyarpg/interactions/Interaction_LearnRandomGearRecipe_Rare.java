...
public class Interaction_LearnRandomGearRecipe_Rare extends SimpleInstantInteraction {
    // Create the CODEC - this is required for serialization
    public static final BuilderCodec<Interaction_LearnRandomGearRecipe_Rare> CODEC = BuilderCodec.builder(
            Interaction_LearnRandomGearRecipe_Rare.class,
            Interaction_LearnRandomGearRecipe_Rare::new,
    ...
    protected void firstRun(
        ...
        // filter the players discovered recipes down to only rares
        List<String> rareRecipes = craftingKnowledge.discoveredDroppableRecipes.stream()
            .filter(s -> s.contains("_Rare"))
            .toList();
        ...
        List<String> commonRecipes = craftingKnowledge.discoveredDroppableRecipes.stream()
            .filter(
                ...
                && !rareRecipes.contains(s.replace("_Common", "_Rare"))
            )
            .toList();
        ...
        craftingKnowledge.addDiscoveredRecipe(entityRef, store, randomRecipe.replace("_Common", "_Rare"));
    }
}
